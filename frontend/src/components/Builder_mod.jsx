      ...
      <body>
        ...
        <div id="contact-modal" class="modal-overlay">
          <div class="modal-content">
            <button class="modal-close" onclick="closeContactFormModal()"></button>
...
const cancelDelete = () => {
  ...
  const alignElements = (alignment) => {
    ...
    // Find targets
    for (let sec of activeLayout) {
      const selectedEls = (sec.elements || []).filter(e => selectedElementIds.includes(e.id));
      if (selectedEls.length > 0) {
        if (alignment === 'left') {
          targetX = Math.min(...selectedEls.map(e => e.x || 0));
        } else if (alignment === 'right') {
          targetX = Math.max(...selectedEls.map(e => (e.x || 0) + (e.width || 0)));
        } else if (alignment === 'center') {
          const minX = Math.min(...selectedEls.map(e => e.x || 0));
          const maxX = Math.max(...selectedEls.map(e => (e.x || 0) + (e.width || 0)));
          centerX = (minX + maxX) / 2;
        }
        break;
      }
    ...
    const nextLayout = activeLayout.map(sec => ({
      ...
      elements: (sec.elements || []).map(el => {
        if (selectedElementIds.includes(el.id)) {
          if (alignment === 'left') return { ...el, x: targetX };
          if (alignment === 'right') return { ...el, x: targetX - (el.width || 0) };
          if (alignment === 'center') return { ...el, x: centerX - (el.width || 0) / 2 };
        }
        return el;
      })
    }));
...
const renderCanvasElement = (el, secId) => {
  ...
  {activePage && activeLayout ? (
    activeLayout.length === 0 ? (
      ...
      activeLayout.map((sec, secIdx) => {
        ...
        return (
          <section key={sec.id} style={{ position: 'relative', width: '100%', minHeight: '400px', ...secStyles }} className={isPreview ? '' : 'builder-canvas-section'}>
            {!isPreview && (
            ...
            <div style={{ maxWidth: containerWidth, margin: '0 auto', padding: '0 20px', boxSizing: 'border-box', position: 'relative', height: '100%', minHeight: '400px' }}
            ...
            {selectedElementIds.length > 1 ? (
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '1px solid var(--border)', paddingBottom: '10px' }}>
                  <h3 style={{ fontSize: '14px', fontWeight: 'bold' }}>Multi-Selection</h3>
                  <span style={{ fontSize: '11px', color: 'var(--primary)', fontWeight: 'bold', padding: '2px 6px', background: 'var(--primary-glow)', borderRadius: '4px' }}>{selectedElementIds.length} ITEMS</span>
                </div>
                <button onClick={handleGroup} style={{ width: '100%', padding: '10px', background: 'var(--primary)', color: '#fff', border: 'none', borderRadius: '4px', marginBottom: '10px', cursor: 'pointer' }}>Group Elements</button>
                <button onClick={handleUngroup} style={{ width: '100%', padding: '10px', background: 'transparent', border: '1px solid var(--border)', color: '#fff', borderRadius: '4px', marginBottom: '20px', cursor: 'pointer' }}>Ungroup</button>
                <h4 style={{ fontSize: '12px', fontWeight: 'bold', color: 'var(--text-secondary)', marginBottom: '10px' }}>Align Elements</h4>
                <div style={{ display: 'flex', gap: '10px' }}>
                  <button onClick={() => alignElements('left')} style={{ flex: 1, padding: '8px', background: '#334155', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Left</button>
                  <button onClick={() => alignElements('center')} style={{ flex: 1, padding: '8px', background: '#334155', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Center</button>
                  <button onClick={() => alignElements('right')} style={{ flex: 1, padding: '8px', background: '#334155', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Right</button>
                </div>
              </div>
            ) : selectedElement ? (
              <div>
  ...
  <div style={{
    ...
    <div style={{
      ...
      <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'center' }}>
        <span style={{ fontSize: '48px', color: '#6366f1' }}></span>
      </div>
